...
class ConvNeXt(Backbone):
    ...
    def __init__(self, in_chans=3, depths=[3, 3, 9, 3], dims=[96, 192, 384, 768], 
                 drop_path_rate=0., layer_scale_init_value=1e-6, out_features=None, frozen_stages=-1):
        super().__init__()
    ...
    def forward_features(self, x):
        ...
        for i in range(4):
            ...
            x = self.stages[i](x)
            if i in self._out_features:  # out features -> FPN in features
...
def build_convnext_backbone(cfg, input_shape: ShapeSpec):
    return ConvNeXt(
